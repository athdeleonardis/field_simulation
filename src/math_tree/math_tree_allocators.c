...
uint64_t _allocate_math_tree_selector(allocator_t *allocator, MATH_NODE_TYPE selector) {
    uint64_t node_id = allocate(allocator, sizeof(math_tree_node_t));
    math_tree_node_t *node = (math_tree_node_t *)allocation_get(allocator, node_id);
    node->type = math_node_type(MATH_VARIABILITY_SELECTOR, MATH_TYPE_SELECTOR, 0, selector);
    return node_id;
}
...
uint64_t math_constant_number(allocator_t *allocator, MATH_NUMBER number) {
    uint64_t node_id = _allocate_math_tree_node(allocator, math_node_type(MATH_VARIABILITY_CONSTANT, MATH_TYPE_NUMBER, 0, 0));
    math_tree_node_t *node = (math_tree_node_t *)allocation_get(allocator, node_id);
    node->number = number;
    return node_id;
}
...
uint64_t math_constant_vector(allocator_t *allocator, MATH_NUMBER x, MATH_NUMBER y, MATH_NUMBER z) {
    uint64_t node_id = _allocate_math_tree_node(allocator, math_node_type(MATH_VARIABILITY_CONSTANT, MATH_TYPE_VECTOR, 0, 0));
    math_tree_node_t *node = (math_tree_node_t *)allocation_get(allocator, node_id);
    node->vector[0] = x;
    node->vector[1] = y;
    node->vector[2] = z;
...
uint64_t math_operator_number_add(allocator_t *allocator, uint64_t left, uint64_t right) {
    MATH_NODE_TYPE type = math_node_type(MATH_VARIABILITY_OPERATOR, MATH_TYPE_NUMBER, MATH_TYPE_NUMBER, MATH_TYPE_NUMBER);
...
uint64_t math_operator_vector_add(allocator_t *allocator, uint64_t left, uint64_t right) {
    MATH_NODE_TYPE type = math_node_type(MATH_VARIABILITY_OPERATOR, MATH_TYPE_VECTOR, MATH_TYPE_VECTOR, MATH_TYPE_VECTOR);
...
uint64_t math_operator_vector_dot(allocator_t *allocator, uint64_t left, uint64_t right) {
    MATH_NODE_TYPE type = math_node_type(MATH_VARIABILITY_OPERATOR, MATH_TYPE_NUMBER, MATH_TYPE_VECTOR, MATH_TYPE_VECTOR);
...
uint64_t math_operator_vector_component(allocator_t *allocator, uint64_t vector, MATH_VECTOR_COMPONENT component) {
    ...
    MATH_NODE_TYPE type = math_node_type(MATH_VARIABILITY_OPERATOR, MATH_TYPE_NUMBER, MATH_TYPE_VECTOR, MATH_TYPE_SELECTOR);
    return _allocate_math_tree_operator(allocator, type, vector, selector, math_function_vector_component);
}

uint64_t math_operator_number_vector_multiply(allocator_t *allocator, uint64_t number, uint64_t vector) {
    MATH_NODE_TYPE type = math_node_type(MATH_VARIABILITY_OPERATOR, MATH_TYPE_VECTOR, MATH_TYPE_NUMBER, MATH_TYPE_VECTOR);
